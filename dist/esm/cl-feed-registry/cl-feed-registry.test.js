...
describe('ChainlinkFeedsRegistry', () => {
    describe('initialize', () => {
        it('Should throw error if contract address is wrong', async () => {
            expect(() => new ChainlinkFeedsRegistry({
    ...
    describe('decimals', () => {
        ...
        it('Should throw error if token address is wrong', async () => {
            await expect(registry.decimals(mockInvalidEthereumAddress, Denominations.eth)).rejects.toThrow('tokenAddress is not valid');
        });
        it('Should get the decimals', async () => {
            const decimals = await registry.decimals(mockValidEthereumAddress, Denominations.eth);
    ...
    describe('latestRoundData', () => {
        ...
        it('Should throw error if token address is wrong', async () => {
            await expect(registry.latestRoundData(mockInvalidEthereumAddress, Denominations.eth)).rejects.toThrow('tokenAddress is not valid');
        });
        it('Should get the decimals', async () => {
            const latestAnswer = await registry.latestRoundData(mockValidEthereumAddress, Denominations.eth);
    ...
    describe('getPriceFeed', () => {
        ...
        it('Should throw error if token address is wrong', async () => {
            await expect(registry.getPriceFeed(mockInvalidEthereumAddress, Denominations.eth)).rejects.toThrow('tokenAddress is not valid');
        });
        it('Should get the decimals', async () => {
            const priceFeed = await registry.getPriceFeed(mockValidEthereumAddress, Denominations.eth);
