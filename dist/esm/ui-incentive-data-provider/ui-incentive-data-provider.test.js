...
import { getReservesIncentivesDataMock, getUserIncentivesDataMock, } from './_mocks';
...
describe('UiIncentiveDataProvider', () => {
    ...
    const createValidInstance = () => {
        ...
        mockGetReservesIncentivesData.mockResolvedValue(getReservesIncentivesDataMock);
    ...
    describe('creating', () => {
        it('should throw an error if the contractAddress is not valid', () => {
            expect(() => new UiIncentiveDataProvider({
    ...
    describe('getFullReservesIncentiveData', () => {
        it('should throw if user is not a valid ethereum address', async () => {
            ...
            await expect(instance.getFullReservesIncentiveData(mockInvalidEthereumAddress, mockValidEthereumAddress)).rejects.toThrow('User address is not a valid ethereum address');
        });
        it('should throw if lending pool address is not a valid ethereum address', async () => {
            const instance = createValidInstance();
            await expect(instance.getFullReservesIncentiveData(mockValidEthereumAddress, mockInvalidEthereumAddress)).rejects.toThrow('Lending pool address provider is not valid');
        });
        it('should not throw if user and lending pool address provider is a valid ethereum address', async () => {
            const instance = createValidInstance();
            await expect(instance.getFullReservesIncentiveData(mockValidEthereumAddress, mockValidEthereumAddress)).resolves.not.toThrow();
        });
    });
    describe('getReservesIncentivesData - to get 100% in coverage :( pointless test', () => {
        it('should throw if lending pool address is not a valid ethereum address', async () => {
            const instance = createValidInstance();
            await expect(instance.getReservesIncentivesData(mockInvalidEthereumAddress)).rejects.toThrow('Lending pool address provider is not valid');
        });
        it('should not throw', async () => {
            const instance = createValidInstance();
            await expect(instance.getReservesIncentivesData(mockValidEthereumAddress)).resolves.not.toThrow();
        });
    });
    describe('getReservesIncentivesDataHumanized', () => {
        it('Should throw error if token address is wrong', async () => {
            const instance = createValidInstance();
            await expect(instance.getReservesIncentivesDataHumanized(mockInvalidEthereumAddress)).rejects.toThrow('Lending pool address provider is not valid');
        });
    });
    describe('getUserReservesIncentivesDataHumanized', () => {
        it('Should throw error if token address is wrong', async () => {
            const instance = createValidInstance();
            await expect(instance.getUserReservesIncentivesDataHumanized(mockInvalidEthereumAddress, mockInvalidEthereumAddress)).rejects.toThrow('Lending pool address provider is not valid');
        });
    ...
    describe('getIncentivesDataWithPrice', () => {
        ...
        it('should work with one feed', async () => {
            ...
            mocked(clInstance).getPriceFeed.mockReturnValue(Promise.resolve({
                answer: '2',
                updatedAt: 4,
                decimals: 1,
            }));
            const result = await instance.getIncentivesDataWithPrice({
                lendingPoolAddressProvider: mockValidEthereumAddress,
                chainlinkFeedsRegistry: mockValidEthereumAddress,
                quote: Denominations.eth,
            });
            expect(clInstance.getPriceFeed).toBeCalled();
            expect(typeof result[0].aIncentiveData.emissionEndTimestamp).toEqual(typeof 1);
        ...
        it('should work with no feed twice', async () => {
            ...
            const result2 = await instance.getIncentivesDataWithPrice({
                lendingPoolAddressProvider: mockValidEthereumAddress,
                chainlinkFeedsRegistry: mockValidEthereumAddress,
                quote: Denominations.eth,
            });
    ...
    describe('getUserReserves', () => {
        it('should throw if lending pool address is not a valid ethereum address', async () => {
            const instance = createValidInstance();
            await expect(instance.getUserReservesIncentivesData(mockValidEthereumAddress, mockInvalidEthereumAddress)).rejects.toThrow('Lending pool address provider is not valid');
        });
        it('should throw if user is not a valid ethereum address', async () => {
            const instance = createValidInstance();
            await expect(instance.getUserReservesIncentivesData(mockInvalidEthereumAddress, mockValidEthereumAddress)).rejects.toThrow('User address is not a valid ethereum address');
        });
        it('should not throw if user is a valid ethereum address', async () => {
            const instance = createValidInstance();
            await expect(instance.getUserReservesIncentivesData(mockValidEthereumAddress, mockValidEthereumAddress)).resolves.not.toThrow();
        });
    });
});
//# sourceMappingURL=ui-incentive-data-provider.test.js.map