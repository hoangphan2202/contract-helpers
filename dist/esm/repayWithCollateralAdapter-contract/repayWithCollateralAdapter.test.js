...
describe('RepayWithCollateralAdapterService', () => {
    ...
    describe('Initialize', () => {
        ...
        it('Expects to initialize the service with all params', () => {
            expect(() => new RepayWithCollateralAdapterService(provider, repayWithCollateralAddress)).not.toThrow();
        });
        it('Expects to initialize the service without repayWithCollateralAddress', () => {
            expect(() => new RepayWithCollateralAdapterService(provider)).not.toThrow();
        });
    });
    describe('swapAndRepay', () => {
        ...
        jest
            ...
            .mockImplementation(async () => Promise.resolve(BigNumber.from(1)));
        const repayInstance = new RepayWithCollateralAdapterService(provider, repayWithCollateralAddress);
        afterEach(() => {
            jest.clearAllMocks();
        });
