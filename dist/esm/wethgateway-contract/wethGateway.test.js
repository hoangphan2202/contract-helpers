...
describe('WethGatewayService', () => {
    ...
    describe('Initialization', () => {
        ...
        it('Expects to be initialized', () => {
            expect(() => new WETHGatewayService(provider, erc20Service, wethGatewayAddress)).not.toThrow();
        });
        it('Expects to initialize without wethgateway address', () => {
            expect(() => new WETHGatewayService(provider, erc20Service)).not.toThrow();
        });
    });
    describe('depositETH', () => {
        ...
        it('Expects the deposit tx object to be correct with all params', async () => {
            var _a;
            const weth = new WETHGatewayService(provider, erc20Service, wethGatewayAddress);
            ...
            const decoded = utils.defaultAbiCoder.decode(['address', 'address', 'uint16'], utils.hexDataSlice((_a = tx.data) !== null && _a !== void 0 ? _a : '', 4));
    ...
    describe('withdrawETH', () => {
        ...
        it('Expects the borrow tx object to be correct with all params and variable stable rate without approval', async () => {
            ...
            expect(tx.from).toEqual(user);
            expect(tx.gasLimit).toEqual(BigNumber.from(1));
            const decoded = utils.defaultAbiCoder.decode(['address', 'uint256', 'uint256', 'uint16'], utils.hexDataSlice((_a = tx.data) !== null && _a !== void 0 ? _a : '', 4));
            expect(decoded[0]).toEqual(lendingPool);
