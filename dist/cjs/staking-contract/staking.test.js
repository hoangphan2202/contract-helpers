...
describe('StakingService', () => {
    ...
    jest
        ...
        .mockImplementation(async () => Promise.resolve(ethers_1.BigNumber.from(1)));
    jest.spyOn(provider, 'getNetwork').mockImplementation(async () => Promise.resolve({
        name: 'homestead',
        chainId: 1,
    }));
    ...
    describe('Initialization', () => {
        it('Expects to be initialized with all params', () => {
            const instance = new index_1.StakingService(provider, {
                TOKEN_STAKING_ADDRESS,
                STAKING_HELPER_ADDRESS,
            });
            expect(instance instanceof index_1.StakingService).toEqual(true);
        });
        it('Expects to be initialized without config', () => {
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            expect(instance instanceof index_1.StakingService).toEqual(true);
        });
    });
    describe('signStaking', () => {
        ...
        it('Expects the permission string to be returned when all params', async () => {
            ...
            jest.spyOn(instance.erc20Service, 'getTokenData').mockReturnValue(Promise.resolve({
                name: 'mockToken',
                decimals,
                symbol: 'MT',
                address: '0x0000000000000000000000000000000000000006',
            }));
            const spy = jest.spyOn(IStakedToken__factory_1.IStakedToken__factory, 'connect').mockReturnValue({
                STAKED_TOKEN: async () => Promise.resolve('0x0000000000000000000000000000000000000006'),
            });
        ...
        it('Expects to fail when user not eth address', async () => {
            ...
            const user = 'asdf';
            await expect(async () => instance.signStaking(user, amount, nonce)).rejects.toThrowError(`Address: ${user} is not a valid ethereum Address`);
        });
        it('Expects to fail when amount not positive', async () => {
            ...
            const amount = '0';
            await expect(async () => instance.signStaking(user, amount, nonce)).rejects.toThrowError(`Amount: ${amount} needs to be greater than 0`);
        });
        ...
        it('Expects to fail when nonce not positive or 0', async () => {
            ...
            const nonce = '-1';
            await expect(async () => instance.signStaking(user, amount, nonce)).rejects.toThrowError(`Amount: ${nonce} needs to be greater or equal than 0`);
        });
    ...
    describe('stakeWithPermit', () => {
        ...
        it('Expects the tx object when all params passed', async () => {
            ...
            const stakeTxObj = await instance.stakeWithPermit(user, amount, signature);
            expect(spy).toHaveBeenCalled();
            expect(stakeTxObj.length).toEqual(1);
            expect(stakeTxObj[0].txType).toEqual(types_1.eEthereumTxType.STAKE_ACTION);
            const tx = await stakeTxObj[0].tx();
            expect(tx.to).toEqual(STAKING_HELPER_ADDRESS);
            expect(tx.from).toEqual(user);
            expect(tx.gasLimit).toEqual(ethers_1.BigNumber.from(1));
            const decoded = ethers_1.utils.defaultAbiCoder.decode(['address', 'uint256', 'uint8', 'bytes32', 'bytes32'], ethers_1.utils.hexDataSlice((_a = tx.data) !== null && _a !== void 0 ? _a : '', 4));
            expect(decoded[0]).toEqual(user);
            expect(decoded[1]).toEqual(ethers_1.BigNumber.from((0, utils_1.valueToWei)(amount, 18)));
            expect(decoded[2]).toEqual(28);
            expect(decoded[3]).toEqual('0x532f8df4e2502bd869fb35e9301156f9b307380afdcc25cfbc87b2e939f16f7e');
            expect(decoded[4]).toEqual('0x47c326dc26eb918d327358797ee67ad7415d871ef7eaf0d4f6352d3ad021fbb4');
            // gas price
            const gasPrice = await stakeTxObj[0].gas();
        ...
        it('Expects to fail when user not eth address', async () => {
            ...
            const user = 'asdf';
            await expect(async () => instance.stakeWithPermit(user, amount, signature)).rejects.toThrowError(`Address: ${user} is not a valid ethereum Address`);
        });
        it('Expects to fail when amount not positive', async () => {
            const instance = new index_1.StakingService(provider, {
                TOKEN_STAKING_ADDRESS,
            ...
            const amount = '0';
            await expect(async () => instance.stakeWithPermit(user, amount, signature)).rejects.toThrowError(`Amount: ${amount} needs to be greater than 0`);
        });
        it('Expects to fail when amount not number', async () => {
            const instance = new index_1.StakingService(provider, {
                TOKEN_STAKING_ADDRESS,
    ...
    describe('stake', () => {
        ...
        const { populateTransaction } = IStakedToken__factory_1.IStakedToken__factory.connect(TOKEN_STAKING_ADDRESS, provider);
        ...
        it('Expects the tx object when all params passed and no onBehalfOf with approval needed', async () => {
            ...
            expect(stakeTxObj[1].txType).toEqual(types_1.eEthereumTxType.STAKE_ACTION);
            const tx = await stakeTxObj[1].tx();
            expect(tx.to).toEqual(TOKEN_STAKING_ADDRESS);
            expect(tx.from).toEqual(user);
            expect(tx.gasLimit).toEqual(ethers_1.BigNumber.from(1));
            const decoded = ethers_1.utils.defaultAbiCoder.decode(['address', 'uint256'], ethers_1.utils.hexDataSlice((_a = tx.data) !== null && _a !== void 0 ? _a : '', 4));
            ...
            expect(decoded[1]).toEqual(ethers_1.BigNumber.from((0, utils_1.valueToWei)(amount, 18)));
            // gas price
            const gasPrice = await stakeTxObj[1].gas();
            expect(gasPrice).not.toBeNull();
            expect(gasPrice === null || gasPrice === void 0 ? void 0 : gasPrice.gasLimit).toEqual(utils_1.gasLimitRecommendations[types_1.ProtocolAction.default].limit);
            expect(gasPrice === null || gasPrice === void 0 ? void 0 : gasPrice.gasPrice).toEqual('1');
        });
        it('Expects to fail when not initialized with TOKEN_STAKING_ADDRESS', async () => {
            const instance = new index_1.StakingService(provider, {
        ...
        it('Expects to fail when user not eth address', async () => {
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            const user = 'asdf';
            await expect(async () => instance.stake(user, amount, onBehalfOf)).rejects.toThrowError(`Address: ${user} is not a valid ethereum Address`);
        });
        it('Expects to fail when onBehalfOf not eth address', async () => {
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            const onBehalfOf = 'asdf';
            await expect(async () => instance.stake(user, amount, onBehalfOf)).rejects.toThrowError(`Address: ${onBehalfOf} is not a valid ethereum Address`);
        });
        it('Expects to fail when amount not positive', async () => {
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            const amount = '0';
            await expect(async () => instance.stake(user, amount, onBehalfOf)).rejects.toThrowError(`Amount: ${amount} needs to be greater than 0`);
        });
        it('Expects to fail when amount not number', async () => {
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            const amount = 'asdf';
    ...
    describe('redeem', () => {
        ...
        it('Expects the tx object when all params passed and specific amount', async () => {
            ...
            expect(redeemTxObj[0].txType).toEqual(types_1.eEthereumTxType.STAKE_ACTION);
            const tx = await redeemTxObj[0].tx();
            expect(tx.to).toEqual(TOKEN_STAKING_ADDRESS);
            expect(tx.from).toEqual(user);
            expect(tx.gasLimit).toEqual(ethers_1.BigNumber.from(1));
            const decoded = ethers_1.utils.defaultAbiCoder.decode(['address', 'uint256'], ethers_1.utils.hexDataSlice((_a = tx.data) !== null && _a !== void 0 ? _a : '', 4));
            ...
            expect(decoded[1]).toEqual(ethers_1.BigNumber.from((0, utils_1.valueToWei)(amount, 18)));
            // gas price
            const gasPrice = await redeemTxObj[0].gas();
            expect(gasPrice).not.toBeNull();
            expect(gasPrice === null || gasPrice === void 0 ? void 0 : gasPrice.gasLimit).toEqual('1');
            expect(gasPrice === null || gasPrice === void 0 ? void 0 : gasPrice.gasPrice).toEqual('1');
        });
        ...
        it('Expects to fail when user not eth address', async () => {
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            const user = 'asdf';
            await expect(async () => instance.redeem(user, amount)).rejects.toThrowError(`Address: ${user} is not a valid ethereum Address`);
        });
        it('Expects to fail when amount not positive', async () => {
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            const amount = '0';
            await expect(async () => instance.redeem(user, amount)).rejects.toThrowError(`Amount: ${amount} needs to be greater than 0`);
        });
        it('Expects to fail when amount not number', async () => {
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            const amount = 'asdf';
    ...
    describe('cooldown', () => {
        ...
        it('Expects the tx object when all params passed', async () => {
            ...
            expect(cooldownTxObj[0].txType).toEqual(types_1.eEthereumTxType.STAKE_ACTION);
            const tx = await cooldownTxObj[0].tx();
            expect(tx.to).toEqual(TOKEN_STAKING_ADDRESS);
            expect(tx.from).toEqual(user);
            expect(tx.gasLimit).toEqual(ethers_1.BigNumber.from(1));
            const decoded = ethers_1.utils.defaultAbiCoder.decode([], ethers_1.utils.hexDataSlice((_a = tx.data) !== null && _a !== void 0 ? _a : '', 4));
            expect(decoded).toEqual([]);
            // gas price
            const gasPrice = await cooldownTxObj[0].gas();
            expect(gasPrice).not.toBeNull();
            expect(gasPrice === null || gasPrice === void 0 ? void 0 : gasPrice.gasLimit).toEqual('1');
            expect(gasPrice === null || gasPrice === void 0 ? void 0 : gasPrice.gasPrice).toEqual('1');
        });
        ...
        it('Expects to fail when user not eth address', () => {
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            const user = 'asdf';
            expect(() => instance.cooldown(user)).toThrowError(`Address: ${user} is not a valid ethereum Address`);
        });
    });
    describe('claimRewards', () => {
        ...
        it('Expects the tx object when all params passed with specific amount', async () => {
            ...
            const spy = jest.spyOn(IStakedToken__factory_1.IStakedToken__factory, 'connect').mockReturnValue({
                populateTransaction,
                REWARD_TOKEN: async () => Promise.resolve('0x0000000000000000000000000000000000000006'),
            });
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            jest
                .spyOn(instance.erc20Service, 'decimalsOf')
            ...
            expect(claimRewardsTxObj[0].txType).toEqual(types_1.eEthereumTxType.STAKE_ACTION);
            const tx = await claimRewardsTxObj[0].tx();
            expect(tx.to).toEqual(TOKEN_STAKING_ADDRESS);
            expect(tx.from).toEqual(user);
            expect(tx.gasLimit).toEqual(ethers_1.BigNumber.from(1));
            const decoded = ethers_1.utils.defaultAbiCoder.decode(['address', 'uint256'], ethers_1.utils.hexDataSlice((_a = tx.data) !== null && _a !== void 0 ? _a : '', 4));
            ...
            expect(decoded[1]).toEqual(ethers_1.BigNumber.from((0, utils_1.valueToWei)(amount, 18)));
            // gas price
            const gasPrice = await claimRewardsTxObj[0].gas();
            expect(gasPrice).not.toBeNull();
            expect(gasPrice === null || gasPrice === void 0 ? void 0 : gasPrice.gasLimit).toEqual('1');
            expect(gasPrice === null || gasPrice === void 0 ? void 0 : gasPrice.gasPrice).toEqual('1');
        });
        ...
        it('Expects to fail when user not eth address', async () => {
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            const user = 'asdf';
            await expect(async () => instance.claimRewards(user, amount)).rejects.toThrowError(`Address: ${user} is not a valid ethereum Address`);
        });
        it('Expects to fail when amount not positive', async () => {
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            const amount = '0';
            await expect(async () => instance.claimRewards(user, amount)).rejects.toThrowError(`Amount: ${amount} needs to be greater than 0`);
        });
        it('Expects to fail when amount not number', async () => {
            const instance = new index_1.StakingService(provider, { TOKEN_STAKING_ADDRESS });
            const amount = 'asdf';
