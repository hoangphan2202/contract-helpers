...
describe('Pool', () => {
    ...
    describe('signERC20Approval', () => {
        ...
        it('Expects the permission string to be returned when all params', async () => {
            ...
            expect(message.deadline).toEqual(ethers_1.constants.MaxUint256.toString());
        });
        ...
        it('Expects to fail when user not eth address', async () => {
            ...
            const user = 'asdf';
            await expect(async () => poolInstance.signERC20Approval({ user, reserve, amount })).rejects.toThrowError(`Address: ${user} is not a valid ethereum Address`);
        });
        it('Expects to fail when reserve not eth address', async () => {
            const poolInstance = new index_1.Pool(provider, { POOL });
            const reserve = 'asdf';
            await expect(async () => poolInstance.signERC20Approval({ user, reserve, amount })).rejects.toThrowError(`Address: ${reserve} is not a valid ethereum Address`);
        });
        it('Expects to fail when amount not positive', async () => {
            const poolInstance = new index_1.Pool(provider, { POOL });
            const amount = '0';
            await expect(async () => poolInstance.signERC20Approval({ user, reserve, amount })).rejects.toThrowError(`Amount: ${amount} needs to be greater than 0`);
        });
        it('Expects to fail when amount not number', async () => {
            const poolInstance = new index_1.Pool(provider, { POOL });
            const amount = 'asdf';
    ...
    describe('supplyWithPermit', () => {
        ...
        it('Expects the tx object if all params ok,', async () => {
            ...
            expect(decoded[4]).toEqual(ethers_1.constants.MaxUint256);
    ...
    describe('swapBorrowRateMode', () => {
        ...
        it('Expects to fail when user not and eth address', () => {
            const poolInstance = new index_1.Pool(provider, config);
            const user = 'asdf';
            expect(() => poolInstance.swapBorrowRateMode({
                user,
                reserve,
                interestRateMode,
    ...
    describe('setUsageAsCollateral', () => {
        ...
        it('Expects to fail when user not and eth address', async () => {
            const poolInstance = new index_1.Pool(provider, config);
            const user = 'asdf';
            expect(() => poolInstance.setUsageAsCollateral({
                user,
                reserve,
                usageAsCollateral,
            })).toThrowError(`Address: ${user} is not a valid ethereum Address`);
    ...
    describe('repayWithATokens', () => {
        ...
        it('Expects the tx object passing all params without onBehalfOf', async () => {
            ...
            const synthetixSpy = jest
                .spyOn(poolInstance.synthetixService, 'synthetixValidation')
                .mockReturnValue(Promise.resolve(true));
            const depositTxObj = await poolInstance.repayWithATokens({
                user,
                reserve,
                amount,
                rateMode,
